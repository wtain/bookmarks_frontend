...
import { addOneMonth, parseDateTimeFromMongo, firstDayOfMonth, getDatePortion, datesEqual, lastDayOfMonth } from './DateTimeUtils';
...
describe('DateTimeUtils tests', () => {
  ...
  it('DateTimeUtils.lastDayOfMonth should return last day of month', () => {
    const result = lastDayOfMonth(new Date("2022-02-15")).toISOString();
    expect(result).toBe(new Date("2022-02-28").toISOString());
  });

  it('DateTimeUtils.lastDayOfMonth should return last day of month - for December', () => {
    const result = lastDayOfMonth(new Date("2022-12-15")).toISOString();
    expect(result).toBe(new Date("2022-12-31").toISOString());
  });
